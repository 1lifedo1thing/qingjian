...
.DESCRIPTION
    ...
    uiAccess 由 -Sign 决定，不用手设 QINGJIAN_UIACCESS：默认（不签）关掉 uiAccess，Server 是普通 exe，任何机器都能起；
...
# uiAccess 跟着 -Sign 走（server\build.rs 读 QINGJIAN_UIACCESS）：只有签名构建才开 uiAccess，
